...
class PostsBundle{
	...
	public function addOne($post){
		...
		$this->hooks->call('post_add', array($post));
	}
	...
	public function deleteOne(){
		...
		$this->redis->multi($this->redis_mode)
			...
			->zRem('posts:publish', $id)
			->zRem('posts:unpublish', $id)
			->del("post:$id")
			->exec();
	...
	public function getAll($limit = array(0, -1), $sort = 'desc'){
		...
		$posts = $this->redis->zRevRangeByScore('posts:publish', '+inf', '-inf', array('limit'=>$limit));
		return $posts;
	}
	...
	public function getAllFromCategory($cat, $limit = array(0, -1), $sort = 'desc'){
		$posts = $this->redis->multi($this->redis_mode)
	...
	public function getIdsByCategory($category){
		return $this->redis->sMembers("posts:cat:$category");
	}
	public function getIdsByTag($tag){
		return $this->redis->sMembers("posts:tag:$tag");
	}
